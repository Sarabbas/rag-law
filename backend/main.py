...
async def startup_event():
    ...
    # Use small mT5 model, preloaded once
    llm = HuggingFaceHub(
        repo_id="google/mt5-small",
        model_kwargs={"temperature": 0.3, "max_length": 128},  # Reduced max_length
...
# WebSocket logic remains the same but optimized for minimal memory
@app.websocket("/ws")
...
#     # Load the mT5-small model as LLM
#     llm = await asyncio.to_thread(lambda: HuggingFaceHub(
#         repo_id="google/mt5-small",  # Updated to mT5-small
#         model_kwargs={"temperature": 0.3, "max_length": 256},  # Lower max_length
#         huggingfacehub_api_token=huggingface_token
#     ))
